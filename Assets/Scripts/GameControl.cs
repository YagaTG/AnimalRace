...
public class GameControl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!gameOver)
        {
            ...
            if (gameOver)
            {
                ...
                winCanvasText.GetComponent<TextMeshProUGUI>().text = GameControl.listOfPlayersPlaying[winner].getName() + " Победил!\n" + GameControl.listOfPlayersPlaying[winner].getName() + " заработал " + FINISH_FIRST_REWARD + " монет!";
            }
        }
    }
    ...
    public void LoadPlayerDataToPlayerObjects()
    {
        for (int i = 0; i < listOfPlayersPlaying.Count; i++)
        {
            ...
            playerObjects[i].GetComponentInChildren<TextMeshProUGUI>().text = "<size=60%>(Игрок " + (i + 1) + ")</size>\n" + listOfPlayersPlaying[i].getName();
