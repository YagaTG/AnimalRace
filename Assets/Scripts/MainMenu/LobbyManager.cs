...
public class LobbyManager : MonoBehaviour
{
    ...
    public List<string> animals = new List<string>(); // An array of animal images/files.
    ...
    void Update()
    {
        ...
        { // Do something when the lobby opens
            ...
            currentMap = -1;
            mapName.GetComponent<TextMeshProUGUI>().text = "Выберите карту";

            selectedPlayerNum = -1;
            currentSelectedPlayerText.GetComponent<TextMeshProUGUI>().text = "Имя игрока:";
    ...
    private void startGame()
    {
        ...
        {
            popup("Выберите корректное число игроков");
            return;
        }

        if (currentMap < 0)
        {
            popup("Выберите корректную карту");
            return;
        }
